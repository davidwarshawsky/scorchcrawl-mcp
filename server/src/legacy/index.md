...
import FirecrawlApp, {
  ...
  type MapOptions,
  type Document,
} from './lib/scorch-client/index.js';
  ...
  inputSchema: {
    ...
    properties: {
      ...
      storeInCache: {
        ...
        default: true,
        description:
          'If true, the page will be stored in the ScorchCrawl index and cache. Setting this to false is useful if your scraping activity may have data protection concerns.',
      },
      maxAge: {
        type: 'number',
        default: 172800000,
...
const SCORCHCRAWL_API_URL = process.env.SCORCHCRAWL_API_URL;
const SCORCHCRAWL_API_KEY = process.env.SCORCHCRAWL_API_KEY;

// Check if API key is required (not needed for cloud service)
if (process.env.CLOUD_SERVICE !== 'true' && !SCORCHCRAWL_API_KEY) {
  console.error('Error: SCORCHCRAWL_API_KEY environment variable is required');
  process.exit(1);
}

// Initialize scraping client with optional API URL

// Configuration for retries and monitoring
const CONFIG = {
  retry: {
    maxAttempts: Number(process.env.SCORCHCRAWL_RETRY_MAX_ATTEMPTS) || 3,
    initialDelay: Number(process.env.SCORCHCRAWL_RETRY_INITIAL_DELAY) || 1000,
    maxDelay: Number(process.env.SCORCHCRAWL_RETRY_MAX_DELAY) || 10000,
    backoffFactor: Number(process.env.SCORCHCRAWL_RETRY_BACKOFF_FACTOR) || 2,
  },
  credit: {
    warningThreshold:
      Number(process.env.SCORCHCRAWL_CREDIT_WARNING_THRESHOLD) || 1000,
    criticalThreshold:
      Number(process.env.SCORCHCRAWL_CREDIT_CRITICAL_THRESHOLD) || 100,
  },
};
  ...
  async function callToolRequestHandler(request) {
    ...
    try {
      ...
      const client = new FirecrawlApp({
        apiKey,
        ...(SCORCHCRAWL_API_URL ? { apiUrl: SCORCHCRAWL_API_URL } : {}),
      });
      ...
      switch (name) {
        case 'scorch_scrape': {
          if (!isScrapeOptions(args)) {
            throw new Error('Invalid arguments for scorch_scrape');
          }
        ...
        case 'scorch_map': {
          if (!isMapOptions(args)) {
            throw new Error('Invalid arguments for scorch_map');
          }
          ...
          if (!response.links) {
            throw new Error('No links received from scraping API');
          }
          return {
            content: [
              {
        ...
        case 'scorch_crawl': {
          if (!isCrawlOptions(args)) {
            throw new Error('Invalid arguments for scorch_crawl');
          }
          const { url, ...options } = args;
        ...
        case 'scorch_check_crawl_status': {
          if (!isStatusCheckOptions(args)) {
            throw new Error(
              'Invalid arguments for scorch_check_crawl_status'
            );
          }
  ...
  response.data.length > 0 ? '\nResults:\n' + formatResults(response.data) : ''
        ...
        case 'scorch_search': {
          if (!isSearchOptions(args)) {
            throw new Error('Invalid arguments for scorch_search');
          }
          try {
            const response = await withRetry(
              async () =>
        ...
        case 'scorch_extract': {
          if (!isExtractOptions(args)) {
            throw new Error('Invalid arguments for scorch_extract');
          }
...
async function runLocalServer() {
  try {
    console.error('Initializing ScorchCrawl MCP Server...');
    ...
    safeLog('info', 'ScorchCrawl MCP Server initialized successfully');
    safeLog(
      'info',
      `Configuration: API URL: ${SCORCHCRAWL_API_URL || 'default'}`
    );

    console.error('ScorchCrawl MCP Server running on stdio');
  } catch (error) {
